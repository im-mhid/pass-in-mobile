  ...
  return(
    ...
    >
      ...
      <View className="bg-black/20 self-stretch items-center pb-6 border border-white/10 mx-3 rounded-2xl -mt-5">
        ...
        >
          <View className="w-full flex-row items-center justify-between gap-6 max-h-5">
            <Text className="text-zinc-50 text-sm font-bold">
              {data.eventTitle}
            </Text>
            <Text className="text-zinc-50 text-sm font-bold max-w-20">
              #{data.id}
            </Text>
          </View>
        ...
        { data.image ? 
          ...
          (
            <TouchableOpacity activeOpacity={0.9} onPress={onChangeAvatar}>
              <Image source={{uri: data.image}} className="w-36 h-36 rounded-full -mt-24 bg-gray-400"/>
            </TouchableOpacity>
          )
          :
          (
            <TouchableOpacity 
              activeOpacity={0.9} 
              style={{width: 144, height: 144, borderRadius: 9999, marginTop: -96, backgroundColor: "rgb(156, 163, 175)", justifyContent: "center", alignItems: "center"}}
        ...
        <TouchableOpacity activeOpacity={0.7} style={{marginTop: 24}} onPress={onExpandQRCode}>
